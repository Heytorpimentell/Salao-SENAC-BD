...
-- Total a pagar de salário aos funcionarios e profissionais.
SELECT SUM(COALESCE(f.salario, 0) + COALESCE(p.salario, 0)) AS Total_salariosPagar
FROM tbl_funcionario f
LEFT JOIN tbl_profissional p ON f.cpf = p.cpf;

-- Total de comissão a pagar aos funcionários e profissionais.
SELECT SUM(COALESCE(f.comissao, 0) + COALESCE(p.comissao, 0)) AS Total_comomissaoPagar
FROM tbl_funcionario f
LEFT JOIN tbl_profissional p ON f.cpf = p.cpf;
